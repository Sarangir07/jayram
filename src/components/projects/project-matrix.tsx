      ...
      <div className="u-container py-[clamp(4.5rem,8vw,8rem)]">
        <Reveal className="max-w-[40rem]">
          ...
          <p className="u-lede mt-5 max-w-[32rem] text-[0.98rem]">
            Every completed site, newest first, by year, location, service and
            sector. Select an entry to open it in the register above.
          </p>
        </Reveal>
        ...
        <Reveal className="mt-12 hidden md:block">
          <table className="w-full border-collapse">
            <caption className="sr-only">
              Completed sites by year, location, service and sector
            </caption>
            <thead>
              <tr className="border-b border-navy text-left">
                <th
                  scope="col"
                  className="w-[4.5rem] pb-3 text-[0.56rem] font-bold tracking-[0.28em] text-muted uppercase"
                >
                  No.
                </th>
                {COLS.map((c) => (
                  <th
                    key={c}
                    scope="col"
                    className="pb-3 text-[0.56rem] font-bold tracking-[0.28em] text-muted uppercase"
                  >
                    {c}
                  </th>
                ))}
              ...
              <tbody key={g.key}>
                <tr>
                  <th
                    ...
                    className="bg-navy px-4 py-3 text-left text-[1.05rem] font-black tracking-[-0.02em] text-white uppercase tabular-nums"
                  >
                    {g.label}
                    <span className="ml-4 text-[0.6rem] font-semibold tracking-[0.26em] text-white/70">
                ...
                {g.projects.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <th
                        scope="row"
                        className="py-5 pr-6 text-left font-extrabold tracking-[-0.02em] text-navy"
                      >
                        <button
                          type="button"
                          onClick={() => onSelect(p.id)}
                          className={cn(rowBtn, "text-[1.05rem]")}
                        >
                          {p.title}
                        </button>
                      </th>
                      <td className="py-5 pr-6 text-[0.9rem] text-ink">
                        {p.location}
                      </td>
        ...
        <div className="mt-10 md:hidden">
          {GROUPS.map((g) => (
            <section key={g.key} aria-label={g.label}>
              <h3 className="bg-navy px-4 py-3 text-[1rem] font-black tracking-[-0.02em] text-white uppercase tabular-nums">
                {g.label}
                <span className="ml-3 text-[0.58rem] font-semibold tracking-[0.26em] text-white/70">
                  {String(g.projects.length).padStart(2, "0")} Sites
                </span>
              </h3>
              <ol className="border-t border-navy">
                {g.projects.map((p) => (
                  <li
                    key={p.id}
                    className="grid grid-cols-[3rem_1fr] gap-x-3 border-b border-navy/10 py-5"
                  >
                    ...
                    <div>
                      <button
                        ...
                        className={cn(
                          rowBtn,
                          "text-[1.1rem] font-extrabold tracking-[-0.02em] text-navy",
                        )}
                      >
                        {p.title}
                      </button>
                      <dl className="mt-3 grid gap-2 text-[0.82rem]">
                        <div className="flex gap-3">
                          <dt className="w-16 shrink-0 text-[0.54rem] font-bold tracking-[0.24em] text-muted uppercase">
                            Location
                          </dt>
                        ...
                        <div className="flex gap-3">
                          ...
                          <dd className="text-[0.62rem] font-bold tracking-[0.2em] text-blue uppercase">
                            {p.service}
                          </dd>
                        </div>
                        <div className="flex gap-3">
                          ...
                          <dd className="text-[0.62rem] font-semibold tracking-[0.2em] text-muted uppercase">
                            {p.sector}
                          </dd>
                        </div>
                      </dl>
